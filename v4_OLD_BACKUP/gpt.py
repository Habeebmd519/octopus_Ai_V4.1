...
try:
    from .intelligence.intent_engine import detect as intelligence_detect
    from .intelligence.location import parse_location, valid_coordinates
...
try:
    ...
    # The local cache remains available when an optional Firestore credential is absent.
    db = None
...
def normalize_place(doc_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if rating < 0 or rating > 5: quality_score -= .3
    spam_terms = ("telegram", "whatsapp group", "affiliate", "guaranteed offer")
    if any(term in (name + " " + description).lower() for term in spam_terms): quality_score -= .6
    return {
        "id": doc_id,
        "name": name,
        "slug": slugify(name),
        ...
        "latitude": latitude if coordinates_available else None,
        "longitude": longitude if coordinates_available else None,
        "coordinatesAvailable": coordinates_available,
...
def build_puter_tool_specs() -> List[Dict[str, Any]]:
    ...
    tools.extend([
        ...
        {"type": "function", "function": {"name": "search_nearby", "description": "Find nearby services from OpenStreetMap. Requires valid user latitude and longitude; when absent, return location_required and ask the user to enable location rather than selecting a city.", "parameters": {"type": "object", "properties": {"query": {"type": "string"}, "category": {"type": "string"}, "lat": {"type": "number"}, "lng": {"type": "number"}, "limit": {"type": "integer", "minimum": 1, "maximum": 12}}, "required": ["query"], "strict": True}}}
    ])
    return tools
...
def v4_tool_services(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not valid_coordinates(lat, lng):
        return {"ok": False, "status": "location_required", "requires_permission": True, "message": "User location is required for this request.", "places": []}
...
def v4_tool_weather(args: Dict[str, Any]) -> Dict[str, Any]:
    lat, lng = args.get("latitude", args.get("lat")), args.get("longitude", args.get("lng"))
...
def v4_tool_news(args: Dict[str, Any]) -> Dict[str, Any]:
    query = build_live_search_query(safe_text(args.get("query")))
    raw = tavily_live_search(query, max_results=max(1, min(8, safe_int(args.get("limit"), 5))))
    evidence = normalize_web_results(raw.get("results", []))
    return {"ok": raw.get("ok", False), "news": [{"title": x["title"], "url": x["url"], "source": x["publisher"], "published_at": x["published_at"], "snippet": x["claim"], "score": x["relevance"], "topic": safe_text(args.get("topic")), "location": safe_text(args.get("location"))} for x in evidence], "evidence": evidence, "error_code": None if raw.get("ok") else "news_unavailable"}


def v4_tool_events(args: Dict[str, Any]) -> Dict[str, Any]:
    raw = tavily_live_search(build_live_search_query(safe_text(args.get("query"))), max_results=max(1, min(8, safe_int(args.get("limit"), 5))))
    evidence = normalize_web_results(raw.get("results", []))
    return {"ok": raw.get("ok", False), "events": [{"name": x["title"], "date": None, "time": None, "location": safe_text(args.get("location")), "city": safe_text(args.get("city")), "district": safe_text(args.get("district")), "category": safe_text(args.get("category")), "source": x["publisher"], "url": x["url"], "price": None, "description": x["claim"]} for x in evidence], "evidence": evidence, "error_code": None if raw.get("ok") else "events_unavailable"}
...
def v4_execute_tool(name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    handlers = {
        ...
        "get_weather": v4_tool_weather,
        "search_news": v4_tool_news,
        "search_events": v4_tool_events,
...
def v4_agent_context_api():
    try:
        ...
        return jsonify(build_puter_agent_context(
            ...
            location_payload=body.get("location") if isinstance(body.get("location"), dict) else None,
        ))
    except Exception as exc:
        debug_log("V4 context error", str(exc))
        return jsonify({"error_code": "context_failed", "message": "Unable to prepare the request context.", "retryable": True}), 500
...
def v4_agent_tool_api():
    try:
        ...
        return jsonify({"error_code": "tool_api_failed", "message": "The tool request could not be completed.", "retryable": True}), 500
...
def chat():
    try:
        ...
        result = build_chat_result(
            ...
            user_lat=user_lat if valid_coordinates(user_lat, user_lng) else None,
            user_lng=user_lng if valid_coordinates(user_lat, user_lng) else None,
            user_location_text=user_location_text,
            location_payload=body.get("location") if isinstance(body.get("location"), dict) else None,
        )
...
def debug_intent_api():
    ...
    location = parse_location(body.get("location") if isinstance(body.get("location"), dict) else body)
    intelligent = intelligence_detect(message, history)
    research = build_research_plan(message, intelligent, location.as_dict())
    return jsonify({
        "message": message,
        "intent": intent,
        "masterIntent": master_intent,
        "language": intelligent["language"],
        "subIntent": intelligent["sub_intent"],
        "confidence": intelligent["confidence"],
        "entities": intelligent["entities"],
        "constraints": intelligent["constraints"],
        "freshness": research["freshness"],
        "locationRequired": intelligent["requires_location"],
        "plannedTools": research["steps"],
