  ...
  function makeTools(apiBase) {
    return [
      ...
      },
      {
        ...
        function: {
          name: "get_weather",
          description: "Get current weather and forecast from the dedicated provider. Use for rain, temperature, humidity, wind, and travel weather; do not guess.",
          parameters: { type: "object", properties: { location: { type: "string" }, latitude: { type: "number" }, longitude: { type: "number" }, forecast_days: { type: "integer", minimum: 1, maximum: 7 } }, strict: true }
        }
      },
      {
        type: "function",
        function: {
          name: "search_news",
          description: "Search current web news. Check sources and dates before saying a story is latest.",
          parameters: { type: "object", properties: { query: { type: "string" }, limit: { type: "integer", minimum: 1, maximum: 8 }, location: { type: "string" } }, required: ["query"], strict: true }
        }
      },
      {
        type: "function",
        function: {
          name: "search_events",
          description: "Discover current events with source evidence. Do not invent event dates, timings, prices, or availability.",
          parameters: { type: "object", properties: { query: { type: "string" }, location: { type: "string" }, limit: { type: "integer", minimum: 1, maximum: 8 } }, required: ["query"], strict: true }
        }
      },
      {
        type: "function",
  ...
  async function ask(options) {
    ...
    const ctx = await jsonFetch(`${cfg.apiBase}/api/agent/context`, {
      ...
      body: JSON.stringify({
        ...
        userLat: Number.isFinite(cfg.userLat) ? cfg.userLat : null,
        userLng: Number.isFinite(cfg.userLng) ? cfg.userLng : null,
