...
The compatible V4 routes now use deterministic intent/entity/freshness/planning modules under `v4/intelligence/`. The agent context exposes a V5 response contract, normalized web evidence, source metadata, structured location context, and research plans. Nearby requests require valid coordinates; `0,0` is always treated as unavailable.

Configure `ALLOWED_ORIGINS` for every production browser origin. Weather is optional and uses `WEATHER_API_URL` / `WEATHER_API_KEY`; if absent, the weather tool reports it as unavailable rather than inventing a forecast.
